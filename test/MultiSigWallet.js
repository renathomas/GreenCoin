...
const {assertRevert, expectThrow} = require('./helpers/utils');

const MultiSigWalletMock = artifacts.require('../contracts/mocks/MultiSigWalletMock.sol');
const TestToken = artifacts.require('../contracts/mocks/TestERC20Token.sol');
    ...
    describe('construction', async () => {
        context('error', async () => {
            it(`should throw if created with more than ${MAX_OWNER_COUNT} owners`, async () => {
                ...
                await assertRevert(MultiSigWalletMock.new(owners, 2));
            });

            it('should throw if created without any owners', async () => {
                await assertRevert(MultiSigWalletMock.new([], 2));
            });

            it('should throw if created without any requirements', async () => {
                await assertRevert(MultiSigWalletMock.new([accounts[0], accounts[1]], 0));
            });

            it('should throw if created with a requirement larger than the number of owners', async () => {
                await assertRevert(MultiSigWalletMock.new([accounts[0], accounts[1], accounts[2]], 10));
            });

            it('should throw if created with duplicate owners', async () => {
                await assertRevert(MultiSigWalletMock.new([accounts[0], accounts[1], accounts[2], accounts[1]], 3));
            });
        });
            ...
            context(`with ${spec.owners.length} owners and requirement of ${spec.requirement}`, async () => {
                ...
                describe('submitTransaction', async () => {
                    it('should throw an error, if sent from not an owner', async () => {
                        await assertRevert(wallet.submitTransaction(receiver, value, [], {from: notOwner}));
                    });

                    it('should throw an error, if sent to a 0 address', async () => {
                        await assertRevert(wallet.submitTransaction(null, value, [], {from: sender}));
                    });
                });
                ...
                describe('confirmTransaction', async () => {
                    it('should throw an error, if confirming the same transaction after submitting it', async () => {
                        ...
                        await assertRevert(wallet.confirmTransaction(transactionId, {from: sender}));
                    });
                    ...
                    if (spec.requirement > 1) {
                        it('should throw an error, if sent from not an owner', async () => {
                            ...
                            await assertRevert(wallet.confirmTransaction(transactionId, {from: notOwner}));
                        });
                        ...
                        it('should throw an error, if confirming the same transaction twice', async () => {
                            ...
                            await assertRevert(wallet.confirmTransaction(transactionId, {from: confirmer}));
                        });
                    }

                    it('should throw an error, if confirming a non-existing transaction', async () => {
                        await assertRevert(wallet.confirmTransaction(12345, {from: spec.owners[0]}));
                    });
                });
                ...
                describe('revokeConfirmation', async () => {
                    if (spec.requirement > 1) {
                        it('should throw an error, if sent from not an owner', async () => {
                            ...
                            await assertRevert(wallet.revokeConfirmation(transactionId, {from: notOwner}));
                        });
                        ...
                        it('should throw an error, if asked to revoke a non-confirmed transaction', async () => {
                            ...
                            await assertRevert(wallet.revokeConfirmation(transactionId, {from: spec.owners[1]}));
                        });
                    }
                    ...
                    it('should throw an error, if asked to revoke an executed transaction', async () => {
                        ...
                        await assertRevert(wallet.revokeConfirmation(transactionId, {from: sender}));
                    });
                });
                ...
                describe('executeTransaction', async () => {
                    ...
                    context('executed', async () => {
                        ...
                        it('should throw, if called again', async () => {
                            await assertRevert(wallet.executeTransaction(transactionId, {from: sender}));
                        });
                    });
                    ...
                    context('not executed', async () => {
                        ...
                        context('not confirmed', async () => {
                            ...
                            it('should throw, if called by a non-confirmed owner', async () => {
                                ...
                                await assertRevert(wallet.executeTransaction(transactionId, {from: notConfirmer}));
                            });
                        });
                        ...
                        context('confirmed', async () => {
                            ...
                            it('should throw an error, if asked to be executed by not an owner', async () => {
                                await assertRevert(wallet.executeTransaction(transactionId, {from: notOwner}));
                            });
            ...
            context(`with ${spec.owners.length} owners and requirement of ${spec.requirement}`, async () => {
                ...
                describe('addOwner', async () => {
                    ...
                    it('should throw an error, if called directly', async () => {
                        await assertRevert(wallet.addOwner(notOwner, {from: spec.owners[0]}));
                    });

                    it('should throw an error, if called by not an owner', async () => {
                        await assertRevert(addOwner(notOwner2, notOwner));
                    });
                ...
                describe('removeOwner', async () => {
                    ...
                    it('should throw an error, if called directly', async () => {
                        await assertRevert(wallet.removeOwner(spec.owners[0], {from: spec.owners[0]}));
                    });

                    it('should throw an error, if called by not an owner', async () => {
                        await assertRevert(removeOwner(spec.owners[0], notOwner));
                    });
                ...
                describe('replaceOwner', async () => {
                    ...
                    it('should throw an error, if called directly', async () => {
                        await assertRevert(wallet.replaceOwner(spec.owners[0], spec.owners[1], {from: spec.owners[0]}));
                    });

                    it('should throw an error, if called by not an owner', async () => {
                        await assertRevert(replaceOwner(spec.owners[0], spec.owners[1], notOwner));
                    });
                ...
                describe('changeRequirement', async () => {
                    ...
                    it('should throw an error, if called directly', async () => {
                        ...
                        await assertRevert(wallet.changeRequirement(requirement, {from: spec.owners[0]}));
                    });

                    it('should throw an error, if called by not an owner', async () => {
                        let requirement = spec.requirement == 1 ? 2 : spec.requirement - 1;
                        await assertRevert(changeRequirement(requirement, notOwner));
                    });
